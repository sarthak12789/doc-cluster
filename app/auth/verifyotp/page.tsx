...
const resendOtp = () => {
  ...
  const interval = setInterval(() => {
    setTimer(prev => {
      if (prev <= 1) {
        ...
        return 0; // Timer finished → enable button again
      }
      return prev - 1;
    });
  }, 1000);

  // Reset OTP boxes if you want
  ...
  if (code.trim().length === 0) {
    setStatus("empty");   // 🔥 new status for empty OTP
    return;
  }
...
>
  {status === "error"
    ...
    : " "}
         ...
         <Button
  onClick={verifyOtp}
  disabled={timer > 0}   // ⛔ Disabled during countdown
